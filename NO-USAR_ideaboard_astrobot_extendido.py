"""
Biblioteca de control IdeaBoard–SPIKE.

Copie este archivo en la raiz de CIRCUITPY con el nombre:
    robot_ideaboard.py

El usuario no debe modificar este archivo. Su programa se escribe en
code.py y termina con:
...
TIMEOUT_NACK_ADVERTENCIA = 0.50
TIMEOUT_NACK_DESCONEXION = 2.00
...
# En modo ruedas, los codigos 000..110 representan correcciones
# de -3 a 3. El codigo 111 se reserva para girar sobre el propio eje.
CODIGO_GIRO_SOBRE_EJE = 7

# 00000011:
# modo ruedas, adelante, velocidad 0 y correccion 0.
...
# ==========================================================
# CINCO SENSORES INFRARROJOS DIGITALES
# LOW = linea negra | HIGH = superficie clara
# Orden fisico usado en diagnosticos, de izquierda a derecha:
# EI, CI, C, CD, ED.
# ==========================================================

# IdeaBoard administra las entradas digitales mediante ib.DigitalIn().
# El pull-up mantiene la entrada en HIGH cuando el modulo no detecta negro.
...
# Los modulos entregan el nivel electrico real:
#   0 / LOW  = linea negra
#   1 / HIGH = fondo blanco
SENSOR_NEGRO_ES_LOW = False

# Correccion fuerte para volver a colocar la linea bajo el sensor central.
# Valores disponibles: -3, -2, -1, 0, 1, 2 y 3.
CORRECCION_LINEA = 1
...
# Diagnostico opcional de los cinco sensores.
MOSTRAR_DIAGNOSTICO_SENSORES = True
INTERVALO_DIAGNOSTICO_SENSORES = 0.30


# ==========================================================
# GIROSCOPIO LSM6DS3TRC
# ==========================================================

i2c = board.I2C()
imu = LSM6DS3TRC(i2c, address=0x6A)
...
# Modelo experimental:
# distancia_medida = INTERCEPTO
#                   + COEF_DISTANCIA * distancia_programada
#                   + COEF_VELOCIDAD * velocidad
CAL_DISTANCIA_INTERCEPTO = -0.81422
CAL_DISTANCIA_COEF_DISTANCIA = 0.97808
CAL_DISTANCIA_COEF_VELOCIDAD = 0.059195

# Protocolo interno para el movimiento por distancia.
...
INTERVALO_DIAGNOSTICO_GIRO = 0.50
...
def cuantizar_correccion(correccion):
    """Redondea al entero mas cercano entre -3 y 3."""
...
# ==========================================================
# CODIFICACION DEL BYTE DE CONTROL
# ==========================================================
...
def codificar_giro(direccion, velocidad_base):
    ...
    return (
        ...
        | (bit_direccion << 6)
        | (indice_velocidad << 3)
        | CODIGO_GIRO_SOBRE_EJE
    )


def codificar_accesorios(
    velocidad_garra,
    velocidad_pala,
    direccion="adelante"
):
...
def establecer_ruedas(velocidad_base, correccion=0, direccion="adelante"):
    establecer_mensaje(
        codificar_ruedas(velocidad_base, correccion, direccion)
    )
...
def establecer_accesorios(
    velocidad_garra,
    velocidad_pala,
    direccion="adelante"
):
    establecer_mensaje(
        codificar_accesorios(
            velocidad_garra,
            velocidad_pala,
            direccion
        )
...
# ==========================================================
# PROTOCOLO LPF2
# Handshake identico a la version estable.
# ==========================================================
...
def send_init_sequence():
    ...
    send_msg([0x40, 0x1D])
    send_msg([0x49, 0x00, 0x00])
    send_msg([0x52] + list(struct.pack("<I", BAUD_DATOS)))
    ...
    send_msg([
        0x99, 0x01
    ] + list(struct.pack("<f", 0.0))
      + list(struct.pack("<f", 120.0)))

    # PCT = 0..100
    send_msg([
        0x99, 0x02
    ] + list(struct.pack("<f", 0.0))
      + list(struct.pack("<f", 100.0)))
    ...
    send_msg([0x88, 0x05, 0x10, 0x00])

    # Un valor DATA8, tres cifras, cero decimales.
    send_msg([
        0x90,
        0x80, 0x01, 0x00, 0x03, 0x00
    ])
...
# ==========================================================
# LECTURA DE LOS CINCO SENSORES
# ==========================================================


def leer_nivel_digital(sensor):
    """Retorna exactamente 0 para LOW y 1 para HIGH."""
    return 1 if sensor.value else 0


def nivel_es_negro(nivel):
    """Retorna True cuando el nivel electrico corresponde a negro."""
...
def leer_sensores_linea():
    """
    Retorna diez valores en orden EI/CI/C/CD/ED:

    - Cinco niveles reales: 0=negro, 1=blanco.
    - Cinco booleanos: True=negro, False=blanco.
    """
    ...
    return (
        ...
        nivel_cd,
        nivel_ed,
        nivel_es_negro(nivel_ei),
        nivel_es_negro(nivel_ci),
        nivel_es_negro(nivel_c),
        nivel_es_negro(nivel_cd),
        nivel_es_negro(nivel_ed)
    )


def correccion_desde_sensores(
    externo_izquierdo,
    centro_izquierdo,
    centro,
    centro_derecho,
    ...
    """
    Retorna una correccion logica o None.

    Los sensores externos se reservan para intersecciones. Cuando alguno
    detecta negro, no se usa correccion infrarroja de seguimiento.

    Si la linea queda bajo centro-derecho, el robot debe volver hacia la
    derecha. Si queda bajo centro-izquierdo, debe volver hacia la izquierda.
    """
    ...
    # Centro solo, ambos internos o linea perdida: conservar giroscopio.
    return None
...
def calcular_correccion_giroscopio(rumbo_actual, rumbo_objetivo):
    error_rumbo = normalizar_angulo(
        rumbo_objetivo - rumbo_actual
    )
...
def aplicar_control_rumbo(
    ...
    rumbo_actual, tiempo_actual, velocidad_angular_z = actualizar_rumbo(
        rumbo_actual,
        tiempo_anterior
    )

    correccion_gyro, error_rumbo = calcular_correccion_giroscopio(
        rumbo_actual,
        rumbo_objetivo
    )
...
# ==========================================================
# AVANCE Y REVERSA POR TIEMPO: SOLO GIROSCOPIO
# ==========================================================


def _mover_tiempo_con_giroscopio(
    segundos,
    velocidad_base,
    direccion_movimiento,
    color_led
...
def avance(segundos, velocidad):
    """Avanza durante un tiempo usando solamente el giroscopio."""
    _mover_tiempo_con_giroscopio(
        segundos,
        velocidad,
        DIRECCION_ADELANTE,
        COLOR_AVANCE
...
def reversa(segundos, velocidad):
    """Retrocede durante un tiempo usando solamente el giroscopio."""
    _mover_tiempo_con_giroscopio(
        segundos,
        velocidad,
        DIRECCION_ATRAS,
...
# ==========================================================
# MOVIMIENTO POR DISTANCIA: ENCODERS + GIROSCOPIO
# ==========================================================
...
def calcular_distancia_interna(distancia_objetivo_cm, velocidad):
    """
    Invierte el modelo experimental para obtener la distancia que debe
    programarse en los encoders.

    medida = -0.81422 + 0.97808 * programada + 0.059195 * velocidad
    """
    distancia_interna = (
        distancia_objetivo_cm
        - CAL_DISTANCIA_INTERCEPTO
        - CAL_DISTANCIA_COEF_VELOCIDAD * velocidad
    ) / CAL_DISTANCIA_COEF_DISTANCIA

    if distancia_interna <= 0:
        raise ValueError(
            "La combinacion de distancia y velocidad queda fuera "
            "del rango util del modelo de calibracion"
        )
...
def _mover_distancia(distancia_cm, velocidad, direccion, color_led):
    ...
    distancia_interna_cm = calcular_distancia_interna(
        distancia_cm,
        velocidad
    )
    rotaciones = distancia_interna_cm / CIRCUNFERENCIA_RUEDA_CM
    ...
    print(
        "Distancia solicitada:", round(distancia_cm, 3), "cm",
        "| Distancia interna:", round(distancia_interna_cm, 3), "cm",
        "| Rotaciones:", round(rotaciones, 3),
        "| Grados:", grados_objetivo,
        "| Velocidad:", velocidad,
        "| Control: calibracion + encoders + giroscopio"
    )
    ...
    try:
        ...
        establecer_ruedas(velocidad, 0, direccion=direccion)
        velocidad_minima = max(1, velocidad - CORRECCION_MAXIMA)
        segundos_estimados = grados_objetivo / float(
            velocidad_minima * 10
        )
        fin = (
            time.monotonic()
...
def mover_garra(segundos, velocidad, direccion="adelante"):
    ejecutar_accesorios(
        segundos,
        velocidad_garra=velocidad,
        velocidad_pala=0,
        direccion=direccion,
        color_led=COLOR_GARRA
    )
...
def mover_pala(segundos, velocidad, direccion="adelante"):
    ejecutar_accesorios(
        segundos,
        velocidad_garra=0,
        velocidad_pala=velocidad,
        direccion=direccion,
        color_led=COLOR_PALA
    )


def mover_garra_pala(
    segundos,
    velocidad_garra,
    velocidad_pala,
    direccion="adelante"
...
# ==========================================================
# GIRO SOBRE EL PROPIO EJE: SOLO GIROSCOPIO
# ==========================================================
...
def girar(direccion, angulo, velocidad_base):
    ...
    tiempo_maximo = max(
        5.0,
        (angulo / 90.0) * TIEMPO_MAXIMO_GIRO_90 * factor_velocidad
    )
    ...
    try:
        while True:
            ...
            _, _, gz = imu.gyro
            velocidad_angular_z = (
                math.degrees(gz) - offset_giroscopio_z
            ) * SIGNO_GIROSCOPIO
            ...
            if MOSTRAR_DIAGNOSTICO_GIRO:
                if (
                    tiempo_actual - ultimo_diagnostico
                    >= INTERVALO_DIAGNOSTICO_GIRO
                ):
                    print(
                        "Giro:", direccion_actual,
                        "| GZ:", round(velocidad_angular_z, 2),
                        "| Angulo:", round(angulo_medido, 2),
                        "| Error:", round(error_angulo, 2),
                        "| Byte:", mensaje_actual
                    )
            ...
            if abs(error_angulo) <= TOLERANCIA_ANGULO_GIRO:
                ...
                velocidad_giro = calcular_velocidad_giro(
                    error_angulo,
                    velocidad_base
                )
            ...
            if tiempo_actual - tiempo_inicio > tiempo_maximo:
                raise RuntimeError(
                    "Tiempo maximo excedido durante girar(); "
                    "ultimo angulo medido: "
                    + str(round(angulo_medido, 2))
                )
...
def clasificar_interseccion(externo_izquierdo_visto, externo_derecho_visto):
    """
    Clasificacion con niveles activos en LOW:

        EI = 1 -> interseccion izquierda
        ED = 1 -> interseccion derecha
        EI = 1 y ED = 1 -> interseccion tipo T
    """
...
def avance_hasta(numero_interseccion, velocidad_base=VELOCIDAD_BASE):
    return _mover_hasta_interseccion(
        numero_interseccion,
        velocidad_base,
        DIRECCION_ADELANTE
    )
        ...
        while connected:
            try:
                ...
                if connected:
                    ...
                    print("Error:", error)
                    print(
                        "El enlace LPF2 sigue activo. "
                        "Se vuelve a esperar el boton."
                    )
                    continue
